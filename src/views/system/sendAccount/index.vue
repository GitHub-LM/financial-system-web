        ...
        <el-row>
          <el-col :span="12">
            <el-form-item
              ...
              prop="modifyContent"
            >
              <el-input v-model="form.modifyContent" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      this.$api.getSendAccountList().then((response) => {
    ...
    handleQuery () {
      ...
      this.loading = true;
      this.$api.getSendAccountList(this.addDateRange(this.queryParams, this.dateRange)).then((response) => {
        this.roleList = response.rows;
        this.total = response.total;
        this.loading = false;
      })
    ...
    handleUpdate (row) {
      ...
      this.$api.getSendAccountListById(id).then((response) => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            this.$api.editSendAccountListById(this.form).then((res) => {
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出所有上报数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        self.$api.exportSendAccount(queryParams).then((res) => {
